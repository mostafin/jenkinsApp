 ...
 stage ("Deploy Application on Azure Kubernetes Service")
 {
  switch (env.BRANCH_NAME) {
    ...
    case "canary":
        ...
        sh("kubectl --namespace=trial-production apply -f k8s/services/")
        sh("kubectl --namespace=trial-production apply -f k8s/canary/")
    ...
    case "master":
        ...
        sh("kubectl --namespace=trial-production apply -f k8s/services/")
        sh("kubectl --namespace=trial-production apply -f k8s/production/")
        sh("echo http://`kubectl --namespace=trial-production get service/${appName} --output=json | jq -r '.status.loadBalancer.ingress[0].ip'` > ${appName}")
        break
